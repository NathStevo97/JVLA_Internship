...
for ANT_ID in ANT_LIST: #for each antenna within ANT_LIST
	print(ANT_ID)
	for SpW_ID in SpW_List: #for each spw folder in ant_id folder
		print(SpW_ID)
		meas_list = os.listdir('/data/jvla/analysis_tec/Bandpass/'+ANT_ID+'/'+SpW_ID+'/Left') #measurement list dependent on each antenna
		...
		for i in range(0,64): #for each of the 64 channels per SpW the following will apply
			...
			for fname in meas_list: 
				data_left = np.loadtxt('/data/jvla/analysis_tec/Bandpass/'+ANT_ID+'/'+SpW_ID+'/Left/'+fname,delimiter=",", unpack=True)
				#for each measurement for the given antenna, load data
				
				bandpass_left = data_left[0,:]
				flag_left = data_left[1,:]
				print(fname, SpW_ID, ANT_ID, 'Left')
			...
			rng_channel_left = max(channel_left) - min(channel_left)
		...
		savename_left = '/data/jvla/analysis_tec/Bandpass/'+ANT_ID+'/'+SpW_ID+'_Left.txt'
...
for ANT_ID in ANT_LIST:
        for SpW_ID in SpW_List:
                meas_list = os.listdir('/data/jvla/analysis_tec/Bandpass/'+ANT_ID+'/'+SpW_ID+'/Right')
                med_right = []
                rng_right = []
		per_dev_right = []
                for i in range(0,64):
                        channel_right = []
                        for fname in meas_list:
                                data_right = np.loadtxt('/data/jvla/analysis_tec/Bandpass/'+ANT_ID+'/'+SpW_ID+'/Right/'+fname,delimiter=",", unpack=True)


                                bandpass_right = np.array(data_right[0,:])
                                flag_right = np.array(data_right[1,:])

                                if flag_right[i] == 0.0:
                                        channel_right.append(bandpass_right[i])
                                print(fname, i, SpW_ID, ANT_ID)
			...
			channel_right_med = np.median(channel_right) #find median value of channel considered
                        ...
                        rng_channel_right = max(channel_right) - min(channel_right)
                ...
                savename_right = '/data/jvla/analysis_tec/Bandpass/'+ANT_ID+'/'+SpW_ID+'_Right.txt'
