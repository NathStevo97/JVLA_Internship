...
for ANT_ID in ANT_LIST:
 #for each spw folder in ant_id folder
	for SpW_ID in SpW_List:
		print(SpW_ID)
		meas_list = sorted(os.listdir('/data/jvla/analysis_tec/Leakage/'+ANT_ID+'/'+SpW_ID+'/Left')) #measurement list dependent on each antenna
		...
		for i in range(0,64): #for each of the 64 channels per SpW the following will apply
			...
			for fname in meas_list: 
				data_left = np.loadtxt('/data/jvla/analysis_tec/Leakage/'+ANT_ID+'/'+SpW_ID+'/Left/'+fname,delimiter=",", unpack=True)
		...
		savename_left = '/data/jvla/analysis_tec/Leakage/'+ANT_ID+'/'+SpW_ID+'_Left.txt'
...
for ANT_ID in ANT_LIST:
 #for each spw folder in ant_id folder
	for SpW_ID in SpW_List:
		print(SpW_ID)
		meas_list = sorted(os.listdir('/data/jvla/analysis_tec/Leakage/'+ANT_ID+'/'+SpW_ID+'/Right')) #measurement list dependent on each antenna
		...
		for i in range(0,64): #for each of the 64 channels per SpW the following will apply
			...
			for fname in meas_list: 
				data_right = np.loadtxt('/data/jvla/analysis_tec/Leakage/'+ANT_ID+'/'+SpW_ID+'/Right/'+fname,delimiter=",", unpack=True)
				#for each measurement for the given antenna, load data
		...
		savename_right = '/data/jvla/analysis_tec/Leakage/'+ANT_ID+'/'+SpW_ID+'_Right.txt'
