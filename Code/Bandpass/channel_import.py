...
for ANT_ID in ANT_LIST: #for each antenna within ANT_LIST
	...
	for SpW_ID in SpW_List: #for each spw folder in ant_id folder
		print(SpW_ID)
		meas_list = os.listdir('/data/jvla/analysis/'+ANT_ID+'/'+SpW_ID+'/Left') #measurement list dependent on each antenna
		...
		for i in range(0,64): #for each of the 64 channels per SpW the following will apply
			...
			for fname in meas_list: 
				data_left = np.loadtxt('/data/jvla/analysis/'+ANT_ID+'/'+SpW_ID+'/Left/'+fname,delimiter=",", unpack=True)
		...
		savename_left = '/data/jvla/analysis/'+ANT_ID+'/'+SpW_ID+'_Left.txt'
...
for ANT_ID in ANT_LIST:
        for SpW_ID in SpW_List:
                meas_list = os.listdir('/data/jvla/analysis/'+ANT_ID+'/'+SpW_ID+'/Right')
                med_right = []
                rng_right = []
		per_dev_right = []
                for i in range(0,64):
                        channel_right = []
                        for fname in meas_list:
                                data_right = np.loadtxt('/data/jvla/analysis/'+ANT_ID+'/'+SpW_ID+'/Right/'+fname,delimiter=",", unpack=True)
			...
			channel_right_med = np.median(channel_right) #find median value of channel considered
                ...
                savename_right = '/data/jvla/analysis/'+ANT_ID+'/'+SpW_ID+'_Right.txt'
